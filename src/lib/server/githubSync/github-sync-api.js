...
export class GitHubSyncAPI {
  ...
  async handleStartService(req, res) {
    try {
      ...
      res.json({ success: true, message: 'Sync service started successfully', status: this.scheduler.getStatus() });
    } catch (error) {
      this.logger.error('Failed to start sync service:', error.message);
      res.status(500).json({ success: false, error: 'Failed to start sync service', message: error.message });
    }
  }
  ...
  async handleStopService(req, res) {
    try {
      ...
      res.json({ success: true, message: 'Sync service stopped successfully' });
    } catch (error) {
      this.logger.error('Failed to stop sync service:', error.message);
      res.status(500).json({ success: false, error: 'Failed to stop sync service', message: error.message });
    }
  }
  ...
  async handleGetStatus(req, res) {
    try {
      const status = {
        ...
        api: { uptime: process.uptime(), memory: process.memoryUsage(), timestamp: new Date().toISOString() }
      };
  ...
  async handleTriggerSync(req, res) {
    try {
      if (!this.scheduler || !this.scheduler.isRunning) {
        return res.status(400).json({ success: false, error: 'Sync service is not running' });
      }
      const { immediate = false } = req.body;
      this.logger.info('Triggering full sync scan via API...');
      await this.scheduler.scanForSyncCandidates();
      res.json({ success: true, message: 'Sync scan triggered successfully', immediate, queueSize: this.scheduler.syncQueue.size });
    } catch (error) {
      this.logger.error('Failed to trigger sync:', error.message);
      res.status(500).json({ success: false, error: 'Failed to trigger sync', message: error.message });
    }
  }
  ...
  async handleTriggerProjectSync(req, res) {
    try {
      ...
      const scheduled = await this.scheduler.triggerSync(projectId, { immediate, priority });
      if (scheduled) {
        res.json({ success: true, message: `Sync triggered for project ${projectId}`, projectId, immediate, priority });
      } else {
        res.json({ success: false, message: `Failed to schedule sync for project ${projectId}`, projectId });
      }
    } catch (error) {
      this.logger.error(`Failed to trigger sync for project ${req.params.projectId}:`, error.message);
      res.status(500).json({ success: false, error: 'Failed to trigger project sync', message: error.message });
    }
  }
  ...
  async handleGetQueue(req, res) {
    try {
      if (!this.scheduler) {
        return res.json({ success: true, queue: [], activeSync: [] });
      }
      const status = this.scheduler.getStatus();
      res.json({ success: true, queue: status.queue, activeSync: status.activeSync, stats: { queueSize: status.stats.queueSize, activeSyncs: status.stats.activeSyncs } });
    } catch (error) {
      this.logger.error('Failed to get queue:', error.message);
      res.status(500).json({ success: false, error: 'Failed to get queue', message: error.message });
    }
  }
  ...
  async handleGetHistory(req, res) {
    try {
      ...
      res.status(500).json({ success: false, error: 'Failed to get history', message: error.message });
    }
  }
  ...
  async handleGetProjectHistory(req, res) {
    try {
      ...
      const history = this.scheduler.getSyncHistory(projectId, parseInt(limit));
      res.json({ success: true, projectId, history });
    } catch (error) {
      this.logger.error(`Failed to get history for project ${req.params.projectId}:`, error.message);
      res.status(500).json({ success: false, error: 'Failed to get project history', message: error.message });
    }
  }
  ...
  async handleCleanupHistory(req, res) {
    try {
      if (!this.scheduler) {
        return res.json({ success: false, error: 'Scheduler not available' });
      }
      const { days = 30 } = req.body;
      const deletedCount = this.scheduler.cleanupHistory(days);
      res.json({ success: true, message: `Cleaned up ${deletedCount} old sync records`, deletedCount, days });
    } catch (error) {
      this.logger.error('Failed to cleanup history:', error.message);
      res.status(500).json({ success: false, error: 'Failed to cleanup history', message: error.message });
    }
  }
  ...
  async handleGetConfig(req, res) {
    try {
      const config = { api: { port: this.config.port, host: this.config.host }, scheduler: this.scheduler ? this.scheduler.config : this.config.schedulerOptions };
      res.json({ success: true, config });
    } catch (error) {
      this.logger.error('Failed to get config:', error.message);
      res.status(500).json({ success: false, error: 'Failed to get config', message: error.message });
    }
  }
  ...
  async handleUpdateConfig(req, res) {
    try {
      ...
      res.json({ success: true, message: 'Configuration updated successfully', config: { api: { port: this.config.port, host: this.config.host }, scheduler: this.scheduler ? this.scheduler.config : this.config.schedulerOptions } });
    } catch (error) {
      this.logger.error('Failed to update config:', error.message);
      res.status(500).json({ success: false, error: 'Failed to update config', message: error.message });
    }
  }
  ...
  async handleGetProjects(req, res) {
    try {
      ...
      res.json({ success: true, projects: enhancedProjects, summary: { total: allProjects.length, enabled: allProjects.filter(p => p.github?.enabled).length, withPendingChanges: pendingProjects.length, currentlySyncing: this.scheduler?.activeSync.size || 0 } });
    } catch (error) {
      this.logger.error('Failed to get projects:', error.message);
      res.status(500).json({ success: false, error: 'Failed to get projects', message: error.message });
    }
  }
...
/** Factory to create and start API server */
...
export async function startSyncAPIServer(mcpClient, options = {}) {
  ...
  process.on('SIGTERM', async () => {
    ...
    if (api.scheduler) { await api.scheduler.stop(); }
    await api.stop();
    process.exit(0);
  });
