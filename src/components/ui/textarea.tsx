...
const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(
  ({ className, maxRows = 8, onChange, ...props }, ref) => {
    ...
    return (
      <textarea
        className={cn(
          "flex h-[2.5em] w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 overflow-y-auto overflow-x-auto resize-none transition-height duration-150 whitespace-pre-wrap break-words",
          className
        )}
        ref={(element) => {
