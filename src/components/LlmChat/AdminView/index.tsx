...
export const AdminView = () => {
  const { projects, activeProjectId, updateProjectSettings, servers } =
    useStore();
  ...
  useEffect(() => {
    ...
    const shouldUpdate =
      !projectServerIds.every((id) => activeServerIds.includes(id)) ||
  ...
  const handleSettingsChange = (settings: Partial<ProjectSettings>) => {
    ...
    if (
      ...
      settings.provider !== activeProject.settings.provider
    ) {
      ...
      updateProjectSettings(activeProject.id, {
        settings: {
          ...
          anthropicApiKey:
            activeProject.settings.anthropicApiKey ||
      ...
      {activeTab === "tools" ? (
        ...
        <>
                ...
                <div>
                  ...
                  <Input
                    ...
                    className={
                      activeProject.settings.provider === "openrouter"
                        ? ""
                        : !activeProject.settings.anthropicApiKey?.trim() &&
                  ...
                  <div className="space-y-2">
                    <select
                      value={
                        ...
                        !getProviderModels(
                          activeProject.settings.provider || "anthropic"
                        ).includes(activeProject.settings.model || "")
                          ? "custom"
                          : activeProject.settings.model
                      }
                      onChange={(e) => {
                        console.log(
                          "Model dropdown onChange event:",
                          e.target.value
                        );
          ...
          <McpConfiguration
            ...
            onServerIdsChange={(mcpServerIds) =>
              handleSettingsChange({ mcpServerIds })
            }
          />
          <Card className="mt-6">
            <CardContent className="p-6">
              ...
              >
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      Are you sure you want to reset the application state? This
                      will delete all projects, conversations, and settings.
